...
private:
  bool running = false;
  double dTime = 0;
  double cenX = WIDTH / 2, cenY = HEIGHT / 2;
  double dAspectRatio = 1;
  Matrix matProj = { 0 };

public:
  double dFovDegrees = 90.0;
  double dNear = 1.0, dFar = 1000.0;
  ...
  void DisplayTriangle(const Vector p1, const Vector p2, const Vector p3, double dLight) {
    int c = 192;
    if (SHADING) {
